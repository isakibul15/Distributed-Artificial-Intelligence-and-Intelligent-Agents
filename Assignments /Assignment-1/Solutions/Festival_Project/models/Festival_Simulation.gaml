...
species Guest skills: [moving] {
    ...
    reflex check_needs when: state = "idle" {
        if (hunger <= 20.0) {
            ...
            write "🍔 " + guest_name + " is HUNGRY! [Hunger: " + hunger + "] - Going to Information Center";
        } else if (thirst <= 20.0) {
            current_need <- "WATER";
            state <- "seeking_info";
            target_location <- info_center.location;
            write "💧 " + guest_name + " is THIRSTY! [Thirst: " + thirst + "] - Going to Information Center";
        }
    }
    ...
    reflex replenish_at_store when: state = "at_store" {
        ...
        target_store <- nil;
        target_location <- nil;
        write "🎉 " + guest_name + " is satisfied and going back to wandering";
    }
