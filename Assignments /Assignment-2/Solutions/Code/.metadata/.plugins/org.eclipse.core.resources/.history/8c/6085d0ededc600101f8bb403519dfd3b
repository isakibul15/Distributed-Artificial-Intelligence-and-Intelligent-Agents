...
global {
    ...
    list<string> ended_auction_ids <- [];   //TO AVOID RACE CONDITIONS - JOINING AN AUCTION AFTER IT ENDED
...
species Guest skills: [moving, fipa] {
            ...
            if length(contents_list) >= 6 {
                ...
                if msg_type = "auction_start" {
                    ...
                    if item_genre in preferred_genres and auction_budget > starting_price * 0.2 and !(auction_id in active_auction_ids) {
                        // Join this auction!
                        if(!(auction_id in ended_auction_ids)){
						...
						//NOT BIDDING HERE DIRECTLY - ONLY THE FIRST GUEST MIGHT GET A CHANCE 
						//WAITING FOR THE PRICE UPDATE SO POTENTIAL GUESTS JOIN THE AUCTION FIRST
                ...
                } else if msg_type = "price_update" and (auction_id in active_auction_ids and !(auction_id in ended_auction_ids) ) {
    ...
    reflex receive_inform when: !empty(informs) {
        loop inform_msg over: informs {
            ...
            write "after auction ended getting" + data[0];
            if length(data) >= 1 {
